...
function SectionHeading() {
  ...
  return (
    <div ref={ref} className="text-center max-w-3xl mx-auto mb-24">
      <motion.div
      ...
      >
        ...
        <span className="text-teal-700 text-xs font-bold tracking-widest uppercase">Our Machines</span>
        <motion.span className="h-[2px] bg-teal-500 block" initial={{ width: 0 }} animate={isInView ? { width: 28 } : {}} transition={{ duration: 0.6, delay: 0.2 }} />
      </motion.div>

      <h2 className="text-4xl md:text-5xl lg:text-6xl font-heading font-black text-[#0a1a1a] mb-6 tracking-tight">
      ...
      <motion.div
        className="h-[2px] bg-gradient-to-r from-transparent via-teal-500 to-transparent mx-auto mb-6 rounded-full"
...
function MachineCard({ machine, index }: { machine: (typeof machines)[0]; index: number }) {
  ...
  const rawY = useTransform(scrollYProgress, [0, 1], [80 * depthFactor, -80 * depthFactor]);
  const smoothY = useSpring(rawY, { stiffness: 65, damping: 18 });

  const rawRotX = useTransform(scrollYProgress, [0, 0.35, 0.65, 1], [10, 0, 0, -10]);
  const smoothRotX = useSpring(rawRotX, { stiffness: 65, damping: 18 });

  const driftDir = index === 0 ? -1 : 1;
  const rawX = useTransform(scrollYProgress, [0, 1], [0, 22 * driftDir]);
  ...
  const entranceX = index === 0 ? -60 : 60;
  const entranceRotY = index === 0 ? -25 : 25;
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-[3rem] overflow-hidden border border-[#004445]/10 hover:border-[#004445]/30 transition-all duration-500 group flex flex-col shadow-sm hover:shadow-[0_20px_60px_rgba(0,68,69,0.12)] relative"
    >
      ...
      <div className="h-96 overflow-hidden flex items-center justify-center relative p-12 bg-[#eaf3f3]/40">
        <motion.div
      ...
      <div className="p-12 flex flex-col flex-grow relative z-20">
        <motion.div
          className="flex flex-col sm:flex-row justify-between items-start mb-10 gap-4 border-b border-[#004445]/10 pb-8"
        ...
        >
          <div>
            <motion.span
              className="text-xs font-bold tracking-widest uppercase text-[#004445] mb-3 block"
            ...
            <h3 className="text-3xl font-black text-[#0a1a1a] leading-tight">{machine.name}</h3>
          </div>
          <motion.span
            className="text-4xl font-extrabold text-[#004445]"
        ...
        <Link
          ...
          className="w-full py-5 rounded-2xl bg-[#004445] text-white font-bold text-lg shadow-[0_0_30px_rgba(0,68,69,0.15)] relative overflow-hidden transition-transform duration-300 hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(0,68,69,0.28)] active:scale-95 group/btn flex justify-center"
        >
          <motion.span
            className="absolute inset-0 pointer-events-none"
            initial={{ x: "-100%" }}
...
export function Machines() {
  ...
  const sectionRotX = useTransform(scrollYProgress, [0, 0.15, 0.85, 1], [5, 0, 0, -5]);
  ...
  return (
    <motion.section
      ref={sectionRef}
      className="py-32 bg-[#f8fafa] border-b border-[#004445]/10 relative overflow-hidden"
    ...
    >
      ...
      <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-teal-300 rounded-full mix-blend-multiply filter blur-[200px] opacity-[0.06] pointer-events-none" />
      <div className="absolute bottom-0 right-1/4 w-[400px] h-[400px] bg-teal-700 rounded-full mix-blend-multiply filter blur-[180px] opacity-[0.06] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <Link
            to="/machines"
            className="inline-flex items-center gap-2 px-10 py-4 bg-white border border-[#004445]/20 text-[#0a1a1a] font-bold rounded-full hover:bg-[#eaf3f3] hover:border-[#004445]/40 transition-all shadow-sm group"
          >
            View All Machines
